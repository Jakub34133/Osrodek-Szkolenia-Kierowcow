...
<body>
    ...
    <?php
        ...
        else
        {
            ...
            if(isset($_GET["nowy"])) {
                $maxId_sql = "EXECUTE dbo.Kurs_NajwiekszeId;";

                $maxId_komenda_sql = sqlsrv_prepare($polaczenie, $maxId_sql);
                if (!$maxId_komenda_sql) {
                    print(print_r(sqlsrv_errors(), true));
                    require_once("../uklad_koniec.php");
                    die();
                }

                $maxId_rezultat = sqlsrv_execute($maxId_komenda_sql);
                // $maxId_rezultat = sqlsrv_query($polaczenie, $maxId_sql);

                $maxId = sqlsrv_fetch_array($maxId_komenda_sql, SQLSRV_FETCH_ASSOC);
                $maxIdKurs = $maxId["IdKurs"] + 1;
                ...
                @Par_Zaplacono = ?,
                @Par_Opis = NULL;";
                $noweIdKurs = $maxIdKurs;
            } else {
                $sql = 
                "EXECUTE dbo.Kurs_Aktualizuj
                @Par_IdKurs = ?, 
                @Par_IdKategoria = ?, 
                @Par_IdKursant = ?, 
                @Par_IdStanKursu = ?, 
